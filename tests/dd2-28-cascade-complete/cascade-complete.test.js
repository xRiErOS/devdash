...
// Sprints → completed und ihre offenen Issues → done. Gegen echte In-Memory-DB.
...
function seed(db) {
  ...
  const m1 = ms('M1', 'active')
  const m2 = ms('M2-control', 'active')
  ...
  const s1 = sprint(m1, 'active')
  const s2 = sprint(m1, 'review')
  const sDone = sprint(m1, 'completed') // bereits terminal → unberührt
  const sCtrl = sprint(m2, 'active')
  ...
  const cDone = issue(s1, 'done')      // schon done → bleibt
...
describe('DD2-28 — Cascade-Complete Milestone', () => {
  ...
  test('Cascade setzt offene Sprints completed + offene Issues done', () => {
    ...
    expect(get('SELECT status FROM backlog WHERE id=?', ids.a).status).toBe('done')
    expect(get('SELECT status FROM backlog WHERE id=?', ids.b).status).toBe('done')
    expect(get('SELECT completed_at FROM backlog WHERE id=?', ids.a).completed_at).not.toBeNull()
    expect(get('SELECT status FROM backlog WHERE id=?', ids.cDone).status).toBe('done')
  })

  test('Control-Meilenstein bleibt unberührt', () => {
    cascadeCompleteMilestone(db, ids.m1)
    expect(get('SELECT status FROM milestones WHERE id=?', ids.m2).status).toBe('active')
    expect(get('SELECT status FROM sprints WHERE id=?', ids.sCtrl).status).toBe('active')
    expect(get('SELECT status FROM backlog WHERE id=?', ids.ctrl).status).toBe('in_progress')
  })

  test('Cascade nur aus active', () => {
    db.prepare("UPDATE milestones SET status='planning' WHERE id=?").run(ids.m1)
