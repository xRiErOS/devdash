/**
 * GF-204 — 01 FOUNDATIONS / 01.10 Design Tokens.
...
const meta = {
  title: '01 FOUNDATIONS/01.10 Design Tokens/Design Tokens',
  // Reife folgt PO-Verdict (po-review.json): alle 10 Exports ok, Tier-01-Gate
  // PASS 2026-06-16 (project_memory 404 CONV-status-reife-flow).
...
function Group2({ title, scope, children }) {
  return (
    <section className="dt-type mb-6" data-ui={`fnd.tokens.type.group.${scope}`}>
      <h3 className="font-display text-sm mb-2">{title}</h3>
      {children}
    </section>
  )
...
function TypeRow({ label, spec, scope, className, children }) {
  return (
    <div className="dt-type__row" data-ui={`fnd.tokens.type.${scope}`}>
...
export const Typography = {
  render: () => (
    <div data-ui="fnd.tokens.typography">
