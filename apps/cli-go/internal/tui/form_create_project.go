...
// slug/name/prefix sind Pflicht; slug (a-z0-9-) und prefix (2-6 A-Z0-9) sind seit
// DD2-232 auch nachträglich änderbar (form_edit_project.go / Projekt-Settings,
// nicht mehr immutable). description optional. Werte werden nach Abschluss per
// keyed GetString gelesen (forms_shared-Konvention).
...
func buildProjectCreateForm() *huh.Form {
	...
	return huh.NewForm(huh.NewGroup(
		huh.NewInput().Key("slug").Title("slug").
			Description("URL slug — a-z 0-9 - (changeable later in Project settings)").Value(&slug).Validate(validateProjectSlug),
