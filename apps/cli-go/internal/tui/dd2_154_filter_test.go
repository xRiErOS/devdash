...
// die Issue-Spalte ließ erledigte Issues stehen → jetzt ebenfalls versteckt
// (Issue-Terminal nach DD2-155: completed).
func TestDD2154_DefaultFiltersHideClosed(t *testing.T) {
	...
	// Issue-Spalte: completed + cancelled versteckt, offene Stati sichtbar.
	if m.fIssue.shown("completed") {
		t.Error("Issue-Filter sollte abgeschlossene (completed) standardmäßig verstecken")
	}
