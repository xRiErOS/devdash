...
func buildMemoryForm() *huh.Form {
	return huh.NewForm(huh.NewGroup(
		huh.NewSelect[string]().Key("category").Title("Category").Options(
			...
			huh.NewOption("Wissen", "knowledge"),
			huh.NewOption("Session-Log", "session_log"),
		),
		huh.NewInput().Key("summary").Title("Summary (Pflicht, max 500)").Validate(nonEmpty),
		huh.NewText().Key("content").Title("Content (optional)"),
