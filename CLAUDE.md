...
| MCP-Notes, Tool-Notes, gen-mcp-notes, MCP-Dokumentation, OKF Tool Reference | `scripts/gen-mcp-notes.mjs` | Generiert MCP-Tool-Doku aus Zod-Introspektion: 1 Concept/Domäne, gebündelt, → OKF `~/Obsidian/Knowledge-Catalogue/dev-wiki/cli-devdash-mcp/mcp-tool-reference/` (einziges Ziel, kein Vault-Output), inkl. §9-Conformance-Scan + `okf-cli.py reindex`; Exit 1 bei Violations. Re-runnbar: `node scripts/gen-mcp-notes.mjs`. Tool-Count-Guard anpassen bei MCP-Änderungen. Details: `README.md` → Abschnitt "MCP-Tool-Dokumentation". |
...
| GLOSSARY.md, GLOSSARY-MAP, Glossar, Begriffe, ubiquitous language, Bounded Context, Surface-Glossar | `GLOSSARY-MAP.md` | Wurzel des Glossar-Netzes: verweist je Surface auf `GLOSSARY.md` (Begriffe impl-frei); Wurzel-`GLOSSARY.md` = übergreifende Begriffe (Issue/Sprint/Meilenstein/User-Story) |
| domain-modeling, Begriff schärfen, Glossar pflegen, Term-Konflikt, ubiquitous language | `~/.claude/skills/domain-modeling/SKILL.md` | model-invoked Glossar-Pfleger: fordert Begriffe heraus, präzisiert, prüft gegen Code, schreibt `GLOSSARY.md` fort |
...
| Welche Docs gibt es, docs/, Doku-Übersicht, alle Dokumente | `docs/INDEX.md` | Generierte Manifest-Tabelle aller `docs/` (Titel/Beschreibung/Pfad). Exhaustiv (der Doku-Index oben ist kuratiert). Pflege via YAML-Frontmatter `title`/`description` + `npm run gen:index -- docs` (Guard: `gen:index:docs:check`) |
...
- **Glossar = nur Begriffe.** `GLOSSARY.md` definiert Begriffe, niemals Pfade/Locations (die wandern bei Refactors). Locations → Router; Decisions → Wissensbasis.
