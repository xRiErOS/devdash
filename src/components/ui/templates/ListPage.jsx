...
/**
 * ListPage — kanonisches, token-sauberes Template (DD-481 Phase 4,
 * Harvest aus src/components/ui/ListPage.jsx, DD-423 Archetyp-Organismus).
 *
 * Vorschreibendes Page-Skelett für filterbare Listen-Views (Backlog, Memories,
 * Trash, …). Statt freier Per-View-Komposition rastet eine View nur noch in die
 * benannten Slots ein. Rahmen läuft über {@link PageShell} (title/actions/
 * content). Die List-Slots stapeln vertikal: filterBar über der collection,
 * emptyState als alternativer Zustand bei leerer collection, bulkBar als
 * Sticky-Bottom-Leiste.
 *
 * TIER-EINSCHÄTZUNG → tierFlag = TEMPLATE. ListPage kennt KEINE Domäne: es weiß
 * nichts von Backlog, Memory, Trash oder einem konkreten Datensatz. Es ist ein
 * reines, domänen-agnostisches Page-Level-Layout-Gerüst aus generischen
 * Layout-Slots (filterBar / collection / emptyState / loadingState / errorState
 * / bulkBar) über die {@link PageShell}- und {@link Grid}-Primitiven. Der
 * einzige „Inhalt" ist der harte Pflicht-Slot-Marker (`data-archetype-error`)
 * bei fehlendem emptyState — Vertrags-Optik, keine Domänen-Logik. Damit gehört
 * es seiner Natur nach in den Template-Tier.
 *
 ...
 * Datenladen. Daten kommen als `collection`-Prop, der Row-Typ als `rowOrganism`,
 * Mutationen über die Slot-Inhalte (Callbacks der View). Ephemerer UI-State:
 * keiner nötig (reines Slot-Rendering). Gehobene Kopplung gegenüber der Quelle:
 * KEINE — die Quelle (src/components/ui/ListPage.jsx) war bereits vollständig
 * presentational. Verfeinerung beim Harvest:
 *  - Importe auf die kanonischen Pfade `../layout/PageShell.jsx` und
 *    `../layout/Grid.jsx` umgestellt.
 *  - Das Pass-Through-Prop `dataUi` wurde gemäß I03/D01 zum parametrisierten
 *    `dataUiScope` mit Default `'list-page'` gehoben (Wurzel-data-ui immer
 *    gesetzt, Sub-Anker gepunktet abgeleitet). Damit kann ein Screen via
 *    dataUiScope='backlog' den ganzen Namespace umbiegen.
 *  - Markup verlustfrei übernommen (gleiche DOM-Struktur, gleiche Klassen).
 *
 * Slot-Vertrag (verbindlich, exakt nach Quelle):
 *  - filterBar    (ReactNode)  → Filterleiste über der collection (role="search").
 *  - collection   (Array)      → Datensätze; je Item ein rowOrganism (role="list").
 *  - emptyState   (ReactNode)  → PFLICHT-Slot: Inhalt bei leerer collection.
 *  - loadingState (ReactNode)  → Vorrang-Slot: Ladezustand (truthy = aktiv).
 *  - errorState   (ReactNode)  → Vorrang-Slot: Fehlerzustand (truthy = aktiv).
 *  - bulkBar      (ReactNode)  → Sticky-Bottom-Massenaktionsleiste (role="toolbar").
 *
 * @param {object} props
 ...
 * @param {string} [props.dataUiScope='list-page'] - Wurzel-data-ui-bereich (I03/D01: parametrisiert).
...
export default function ListPage({
  ...
  if (loadingState) {
    body = (
      <div data-ui={`${dataUiScope}.loadingState`} data-slot="loadingState" role="status">
        {loadingState}
      </div>
    )
  } else if (errorState) {
    body = (
      <div data-ui={`${dataUiScope}.errorState`} data-slot="errorState" role="alert">
        {errorState}
      </div>
    )
  } else if (isEmpty) {
    body = emptyState ? (
      <div data-ui={`${dataUiScope}.emptyState`} data-slot="emptyState" role="status">
        {emptyState}
      </div>
    ) : (
      <div
        data-ui={`${dataUiScope}.emptyState`}
    ...
    body = (
      <Grid
        data-ui={`${dataUiScope}.collection`}
  ...
  const content = (
    <div className="flex flex-col gap-3" data-archetype="ListPage" data-ui={dataUiScope}>
      {filterBar && (
        <div data-ui={`${dataUiScope}.filterBar`} data-slot="filterBar" role="search">
          {filterBar}
        </div>
      )}
      {body}
