/**
 * GF-213 — 01 FOUNDATIONS / 01.20 Iconography.
...
const meta = {
  title: '01 FOUNDATIONS/01.20 Iconography/Iconography',
...
// Rollen-Chips = Pill-Atom (outline/neutral), nicht Eigenbau-Markup (PO: keine
// undokumentierten Atome erfinden — Card/Pill sind die kanonischen Bausteine).
function RoleChips({ roles }) {
  return (
    <span className="flex flex-wrap gap-1 justify-center">
      {roles.map((r) => (
        <Pill key={r} variant="outline" color="neutral" size="sm">{r}</Pill>
      ))}
    </span>
  )
}

// Cell = Card-Atom (tone surface0). data-ui override greift über Card's {...rest}.
function Cell({ name }) {
